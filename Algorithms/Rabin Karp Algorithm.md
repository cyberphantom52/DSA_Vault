Rabin-Karp algorithm is used to determine if a pattern $s$ appears in the text $t$ and if it does, then we have to enumerate all its occurrences in $O(|s| + |t|)$ time.
